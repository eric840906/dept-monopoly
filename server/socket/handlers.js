...
const authorizeHost = (socket) => {
  ...
  console.log(
    '  Headers:',
    Object.keys(socket.handshake.headers).filter((k) => k.includes('token'))
  )

  const isAuthorized = socket.handshake.auth?.hostToken === hostToken || socket.handshake.headers['x-host-token'] === hostToken
  ...
  io.on(SOCKET_EVENTS.CONNECTION, (socket) => {
    ...
    socket.on(SOCKET_EVENTS.PLAYER_JOIN, (data) => {
      try {
        ...
        if (!checkRateLimit(socket.id, 'player_join', 10, 60000)) {
    ...
    socket.on(SOCKET_EVENTS.TEAM_JOIN, (data) => {
      try {
        ...
        if (!checkRateLimit(socket.id, 'team_join', 10, 60000)) {
    ...
    socket.on(SOCKET_EVENTS.DICE_ROLL, (data) => {
      try {
        ...
        if (!checkRateLimit(socket.id, 'dice_roll', 10, 10000)) {
    ...
    socket.on(SOCKET_EVENTS.MINI_GAME_SUBMIT, (data) => {
      try {
        ...
        if (!checkRateLimit(socket.id, 'mini_game_submit', 10, 10000)) {
    ...
    socket.on(SOCKET_EVENTS.HOST_CONTROL, (data) => {
      try {
        ...
        if (!checkRateLimit(socket.id, 'host_control', 30, 60000)) {
        ...
        // Handle authentication test separately (before regular host authorization)
        if (sanitizedAction === 'test_auth') {
          const testToken = data.token
          if (!testToken || testToken !== process.env.HOST_TOKEN) {
            socket.emit(SOCKET_EVENTS.ERROR, { message: '無效的主持人令牌' })
            return
          }
          socket.emit('host_control_success', { message: '驗證成功' })
          return
        }
