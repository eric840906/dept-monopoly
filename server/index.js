...
const allowedOrigins = process.env.NODE_ENV === 'production' 
  ? baseOrigins.length > 0 ? baseOrigins : ['https://yourdomain.com']
  : [
      'http://localhost:3000', 
      'http://127.0.0.1:3000',
      ...baseOrigins,
  ...
  contentSecurityPolicy: {
    directives: {
      ...
      scriptSrc: [
        ...
        ...(isDevelopment ? ["'unsafe-eval'", "http://localhost:*", ...networkSources] : [])
      ],
      ...
      connectSrc: [
        ...
        "ws:", 
        "wss:",
        ...(isDevelopment ? ["http://localhost:*", "ws://localhost:*", ...networkSources] : [])
      ],
...
app.use('*', (req, res, next) => {
  ...
  if (req.originalUrl.includes('/js/') || 
      req.originalUrl.includes('/scenes/') || 
      req.originalUrl.includes('/ui/') || 
      req.originalUrl.includes('/mobile/') ||
      req.originalUrl.includes('/sprites/') ||
    ...
    const possiblePaths = [
      ...
      path.join(__dirname, '../client', req.originalUrl),
      path.join(__dirname, '../public', req.originalUrl)
    ];
